...
def test_parse_pdf_returns_fallback_for_unimplemented_banks(monkeypatch) -> None:
    ...
    assert nubank.error is True
    assert nubank.message == "Not implemented yet for bank: BankEnum.NUBANK"
    assert nubank.expenses == []
    assert nubank.allocation == allocation_schema
    assert nubank.reference_year == 2026
    assert nubank.reference_month == 7

    assert caixa.error is True
    assert caixa.message == "Not implemented yet for bank: BankEnum.CAIXA"
