...
Service = Annotated[AllocationContributionService, Depends(allocation_contribution_service)]
...
@router.get(
    "",
    response_model=CustomLimitOffsetPage[AllocationContributionSchema] | list[AllocationContributionSchema],
...
@router.get("/{param}", response_model=AllocationContributionSchema, status_code=HTTPStatus.OK)
async def find_one(
    param: str,
    service: Service,
    current_user: CurrentUser,
...
@router.post("", response_model=AllocationContributionSchema, status_code=HTTPStatus.CREATED)
async def create(service: Service, current_user: CurrentUser, payload: PayloadAllocationContributionCreateSchema):
    return await service.create(current_user=current_user, payload=payload)

@router.put("/{param}", response_model=AllocationContributionSchema, status_code=HTTPStatus.CREATED)
async def update(
        param: str,
        service: Service,
        current_user: CurrentUser,
